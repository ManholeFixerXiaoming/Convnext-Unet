...
class cqm_unetLayer(nn.Module):  # ：B,C,H,W
    def __init__(self, input_channel, drop_path1=0,padd=1, padd2=0):
        ...
        self.bn=nn.BatchNorm2d(input_channel,affine=False)
        self.bn0 = nn.BatchNorm2d(128,affine=False)
        self.bn00 =nn.BatchNorm2d(10,affine=False)
        self.bn1 = nn.BatchNorm2d(256,affine=False)
        self.bn3 = nn.BatchNorm2d(512,affine=False)
        self.bn5 = nn.BatchNorm2d(1024,affine=False)
        self.bn7 = nn.BatchNorm2d(2048,affine=False)
        self.bn9 = nn.BatchNorm2d(4096,affine=False)
        self.conv1 = nn.Conv2d(input_channel, 256, kernel_size=(3, 3), padding=padd, stride=1)
        # self.conv2 = nn.Conv2d(64, 64, kernel_size=(3, 3), padding=padd, stride=1)
        self.conv2 = convnext.Block(256, drop_path=drop_path1, layer_scale_init_value=1e-6)
        self.down1 = nn.MaxPool2d(kernel_size=2, stride=2, padding=padd2)
        self.conv3 = nn.Conv2d(256, 512, kernel_size=(3, 3), padding=padd, stride=1)
        # self.conv4 = nn.Conv2d(128, 128, kernel_size=(3, 3), padding=padd, stride=1)
        self.conv4 = convnext.Block(512, drop_path=drop_path1, layer_scale_init_value=1e-6)
        self.down2 = nn.MaxPool2d(kernel_size=2, stride=2, padding=padd2)
        self.conv5 = nn.Conv2d(512, 1024, kernel_size=(3, 3), padding=padd, stride=1)
        self.conv6 = nn.Conv2d(1024, 1024, kernel_size=(3, 3), padding=padd, stride=1)
        self.conv6 = convnext.Block(1024, drop_path=drop_path1, layer_scale_init_value=1e-6)
        self.down3 = nn.MaxPool2d(kernel_size=2, stride=2, padding=padd2)
        self.conv7 = nn.Conv2d(1024, 2048, kernel_size=(3, 3), padding=padd, stride=1)
        # self.conv8 = nn.Conv2d(512, 512, kernel_size=(3, 3), padding=padd, stride=1)
        self.conv8 = convnext.Block(2048, drop_path=drop_path1, layer_scale_init_value=1e-6)
        self.down4 = nn.MaxPool2d(kernel_size=2, stride=2, padding=padd2)
        self.conv9 = nn.Conv2d(2048, 4096, kernel_size=(3, 3), padding=padd, stride=1)
        # self.conv10 = nn.Conv2d(1024, 1024, kernel_size=(3, 3), padding=padd, stride=1)
        self.conv10 = convnext.Block(4096, drop_path=drop_path1, layer_scale_init_value=1e-6)
        self.up1 = nn.ConvTranspose2d(4096, 2048, kernel_size=2, stride=2, padding=padd2)
        self.up_conv1 = nn.Conv2d(4096, 2048, kernel_size=(3, 3), padding=padd, stride=1)
        # self.up_conv2 = nn.Conv2d(512, 512, kernel_size=(3, 3), padding=padd, stride=1)
        self.up_conv2 = convnext.Block(2048, drop_path=drop_path1, layer_scale_init_value=1e-6)
        self.up2 = nn.ConvTranspose2d(2048, 1024, kernel_size=2, stride=2, padding=padd2)
        self.up_conv3 = nn.Conv2d(2048, 1024, kernel_size=(3, 3), padding=padd, stride=1)
        # self.up_conv4 = nn.Conv2d(256, 256, kernel_size=(3, 3), padding=padd, stride=1)
        self.up_conv4 = convnext.Block(1024, drop_path=drop_path1, layer_scale_init_value=1e-6)
        self.up3 = nn.ConvTranspose2d(1024, 512, kernel_size=2, stride=2, padding=padd2)
        self.up_conv5 = nn.Conv2d(1024, 512, kernel_size=(3, 3), padding=padd, stride=1)
        # self.up_conv6 = nn.Conv2d(128, 128, kernel_size=(3, 3), padding=padd, stride=1)
        self.up_conv6 = convnext.Block(512, drop_path=drop_path1, layer_scale_init_value=1e-6)
        self.up4 = nn.ConvTranspose2d(512, 256, kernel_size=2, stride=2, padding=padd2)
        self.up_conv7 = nn.Conv2d(512, 256, kernel_size=(3, 3), padding=padd, stride=1)
        # self.up_conv8 = nn.Conv2d(64, 64, kernel_size=(3, 3), padding=padd, stride=1)
        self.up_conv8 = convnext.Block(256, drop_path=drop_path1, layer_scale_init_value=1e-6)
        self.up_conv9 = nn.Conv2d(256, 128, kernel_size=(3, 3), padding=padd, stride=1)
        self.up_conv10 = nn.Conv2d(128, 10, kernel_size=(1, 1), padding=padd2, stride=1)
        self.up_conv11 = convnext.Block(10, drop_path=drop_path1, layer_scale_init_value=1e-6)
        self.jh = nn.ReLU()
    def forward(self, x):
        # x = self.conv1(x)
        # x1 = self.conv2(x)
